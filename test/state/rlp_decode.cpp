...
bool decode_header(bytes_view& input, Header& out) noexcept
{
    ...
    if (prefix < SHORT_STRING_OFFSET)  // [0x00, 0x7f] a single byte is its own encoding.
    {
        out = {1, false};
        return true;
    }
    else if (prefix <= LONG_STRING_OFFSET)  // [0x80, 0xb7] short string
    {
        const uint8_t len = prefix - SHORT_STRING_OFFSET;
        if (len >= input_len)
            return false;

        // Canonicality: a single byte < 0x80 must be encoded as itself, not as
        // a 1-byte string with the 0x81 prefix.
        if (len == 1 && input[1] < SHORT_STRING_OFFSET)
            return false;
    ...
    else if (prefix < SHORT_LIST_OFFSET)  // [0xb8, 0xbf] long string
    {
        const uint8_t len_of_str_len = prefix - LONG_STRING_OFFSET;
    ...
    else if (prefix <= LONG_LIST_OFFSET)  // [0xc0, 0xf7] short list
    {
        const uint8_t list_len = prefix - SHORT_LIST_OFFSET;
    ...
    {
        const uint8_t len_of_list_len = prefix - LONG_LIST_OFFSET;
