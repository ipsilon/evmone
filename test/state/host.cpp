...
bool Host::selfdestruct(const address& addr, const address& beneficiary) noexcept
{
    ...
    // EIP-7708: transfer log when SELFDESTRUCT moves value to a different address.
    // No log on self-destruct-to-self (nerf preserves balance, nothing is transferred or burned).
    if (m_rev >= EVMC_AMSTERDAM && balance != 0 && addr != beneficiary)
        emit_transfer_log(m_logs, addr, beneficiary, balance);
