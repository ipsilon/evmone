...
evmc::Result Host::execute_message(const evmc_message& msg) noexcept
{
    ...
    if (m_rev >= EVMC_AMSTERDAM && msg.depth == 0)
    {
        ...
        if (!evmc::is_zero(msg.value) && !recipient_alive)
        {
            ...
            // that the reservoir plus execution gas cover this, rolling authorizations back
            // otherwise.
...
evmc_access_status Host::access_account(const address& addr) noexcept
{
    ...
    // Use the "for access" insertion variant: lazy placeholder without a StateView fetch (keeps
    // trackers from observing accesses that may end up reverted, per EIP-7928 BAL semantics).
