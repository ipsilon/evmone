...
/// chain id (Transaction::chain_id); pre-155 v in {27, 28} yields chain id 0. Because of this
/// normalization, re-encoding a decoded legacy transaction does not reproduce the input bytes (the
/// encoder writes v verbatim). Returns std::nullopt if @p data is malformed or is not exactly one
