...
TransactionCost compute_tx_intrinsic_cost(evmc_revision rev, const Transaction& tx) noexcept
{
    ...
    // EIP-8037: Amsterdam splits auth cost: regular 7500, state (NEW_ACCOUNT+AUTH_BASE)*CPSB per tuple.
    ...
    const auto auth_state_cost = (rev >= EVMC_AMSTERDAM) ?
        static_cast<int64_t>(tx.authorization_list.size()) *
            (STATE_BYTES_PER_NEW_ACCOUNT + STATE_BYTES_PER_AUTH_BASE) * cpsb : int64_t{0};
    ...
    evmc_revision rev = EVMC_PRAGUE, int64_t cpsb = 0)
{
    ...
    for (const auto& auth : authorization_list)
    {
        ...
        //
        // Amsterdam (EIP-8037, bal-devnet-7 PRs #2816 / #2836 / #2848):
    ...
    int64_t block_gas_left, int64_t blob_gas_left, int64_t state_block_gas_left) noexcept
{
    ...
    // EIP-8037 per-tx 2D inclusion check: for each dimension the worst-case
    // contribution must fit in the remaining budget. EELS reference:
    // src/ethereum/forks/amsterdam/fork.py:561-583.
    if (rev >= EVMC_AMSTERDAM)
    {
        // Worst-case regular contribution: tx.gas - intrinsic_state, capped
        // at TX_MAX_GAS_LIMIT (the per-tx regular reservoir).
        const auto worst_case_regular =
            std::min<int64_t>(MAX_TX_GAS_LIMIT, tx.gas_limit - intrinsic_state);
        if (worst_case_regular > block_gas_left)
            return make_error_code(GAS_LIMIT_REACHED);

        // Worst-case state contribution: tx.gas - intrinsic_regular (no cap;
        // the entire execution gas can spill into the state reservoir).
        const auto worst_case_state = tx.gas_limit - intrinsic_cost;
        if (worst_case_state > state_block_gas_left)
            return make_error_code(GAS_LIMIT_REACHED);
    }
    ...
    const TransactionProperties& tx_props)
{
    ...
    const auto delegation_refund =
        process_authorization_list(state, tx.chain_id, tx.authorization_list, rev,
            rev >= EVMC_AMSTERDAM ? COST_PER_STATE_BYTE : int64_t{0});
    ...
    // state-gas charge — no account was created. The refund is added to
    // the reservoir AND tracked in `tx_state_refund` so block-level
    // state gas excludes it.
    ...
    // EIP-8037 (bal-devnet-7, ethereum/execution-specs#2845): All SELFDESTRUCT
    // state gas refunds have been removed. Accounts created and destroyed in
    ...
    if (rev >= EVMC_AMSTERDAM)
    {
        ...
        // from `block.state_gas_used`, so block-level 2D accounting
        ...
        // EIP-8037 (bal-devnet-7, PR #2823): on CREATE address collision the
        ...
        const auto regular_gas = std::max(int64_t{0},
            total_consumed + state_refund - intrinsic_state - exec_state_gas);
