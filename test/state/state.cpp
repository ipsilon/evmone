    ...
    const TransactionProperties& tx_props)
{
    ...
    // Split execution gas into a regular budget and a state-gas reservoir (EIP-8037):
    //   regular = min(MAX_TX_GAS_LIMIT - intrinsic_regular, exec_gas), reservoir = the rest.
    ...
    if (!top_frame_halted)
    {
        ...
        assert(net >= 0);
        exec_state_gas = auth.state_charge + net;
    }

    // Gas consumed = gas_limit - regular_unspent - reservoir_unspent, pre-refund and pre-floor.
    ...
    // `auth.regular_refund` is the flat EIP-7702 refund, which Amsterdam replaces with charges.
    ...
    // dimension, so all of the gas the sender paid for is regular.
    auto block_regular_gas = sender_gas_cost;
    int64_t block_state_gas = 0;
    if (rev >= EVMC_AMSTERDAM)
    {
        // The intrinsic state gas is zero, so whatever `exec_state_gas` does not cover is the
        // regular component, floored so state-gas spending cannot discount it (EIP-7778).
        block_state_gas = exec_state_gas;
        block_regular_gas = std::max(gas_used_b4_refund - exec_state_gas, tx_props.min_gas_cost);
    }
    ...
    receipt.block_regular_gas = block_regular_gas;
