...
TransactionCost compute_tx_intrinsic_cost_amsterdam(const Transaction& tx) noexcept
{
    ...
    if (is_create)
    {
        ...
        recipient_execution = CREATE_ACCESS;
        init_code_gas = INITCODE_WORD_COST * num_words(tx.data.size());
    }
    else if (!is_self_transfer)
    {
        recipient_execution = instr::cold_account_access_cost_amsterdam;
        if (has_value)
    ...
    const TransactionProperties& tx_props)
{
    ...
    // so the reservoir is returned whole — the halt consumes at most the execution budget
    ...
    if (!top_frame_halted)
    {
        ...
        const auto net = message.state_gas - result.state_gas_left + result.state_gas_spilled;
        assert(net >= 0);
        tx_state_gas = auth.state_charge + net;
    }
