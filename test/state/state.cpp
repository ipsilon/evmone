...
{
    // For a create transaction the recipient is the deployment address,
    // computed from the sender and its pre-bump nonce (tx.nonce is validated
    // to equal it).
    const auto recipient =
        tx.to.has_value() ? *tx.to : compute_create_address(tx.sender, tx.nonce);
    ...
    return {
        ...
        .create2_salt = {},
        .code_address = tx.to.value_or(evmc::address{}),
        .code = nullptr,
        .code_size = 0,
    };
}
            ...
            [&](const auto& e) {
                ...
                else if constexpr (std::is_same_v<T, JournalCreate>)
                {
                    ...
                    else
                    {
                        ...
                        //
                        // The same applies to storage slots warmed via the tx
                        // access list (never journaled). The failed frame's own
                        // slot accesses have already been rolled back (LIFO), so
                        // entries still warm here were warmed outside the frame
                        // and must survive as warm-only placeholders, mirroring
                        // insert()'s placeholder promotion.
                        auto& a = modified_get(e.addr);
                        const auto was_warm = (a.access_status == EVMC_ACCESS_WARM);
                        auto storage = std::move(a.storage);
                        std::erase_if(storage, [](const auto& kv) {
                            return kv.second.access_status != EVMC_ACCESS_WARM;
                        });
                        a = Account{};
                        a.loaded = true;
    ...
    int64_t block_gas_left, int64_t blob_gas_left, int64_t state_block_gas_left) noexcept
{
    ...
    // Pre-Amsterdam: the tx must fit in the remaining cumulative block gas.
    // EIP-8037 replaces this with the per-dimension rule-2 checks below; a
    // tx.gas_limit exceeding the whole block.gas_limit is caught there too,
    // via `tx.gas_limit > state_block_gas_left` (<= block.gas_limit), with
    // the same error code.
    ...
    const TransactionProperties& tx_props)
{
    ...
    // Build the message up front so a create transaction's deployment address is
    // computed once (build_message derives it from the sender nonce) and reused
    // for warming below.
    ...
    // Warm the recipient; for a create transaction this is the deployment
    // address (per EIP-2929 the access to it is never reverted).
    ...
    if (!top_frame_halted && tx.to.has_value())
    {
        if (const auto delegate = get_delegate_address(host, *tx.to))
        {
            ...
            // EIP-7928 / EIP-2780: resolving the top-level delegation reads the target's code, so it
            // must appear in the block access list even if the call reverts. Force the lazy-load now
            // (matches get_target_address for sub-call delegation resolution).
    ...
    // gas_refund is derived from it), mutated to the post-refund value pre-Amsterdam.
    ...
    if (rev >= EVMC_AMSTERDAM)
    {
        ...
        // CREATE-collision burned gas — is the regular component. The block header formula
