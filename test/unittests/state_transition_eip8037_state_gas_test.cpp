...
/// The same for a create transaction: TX_BASE plus CREATE_ACCESS instead of the recipient access.
constexpr int64_t AMSTERDAM_CREATE_INTRINSIC = 24'000;
...
TEST_F(state_transition, eip8037_create_tx_revert_refunds_spilled_new_account_charge)
{
    ...
    // The 5-byte initcode's calldata floor exceeds what the reverting frame executes, so the
    // floor is what the sender pays. NEW_ACCOUNT is refunded.
    expect.gas_used = AMSTERDAM_CREATE_INTRINSIC + 5 * 4 * 16;
...
TEST_F(state_transition, eip8037_create_tx_with_value_pays_new_account_once)
{
    ...
    expect.gas_used = AMSTERDAM_CREATE_INTRINSIC + NEW_ACCOUNT_STATE_GAS;
...
TEST_F(state_transition, eip8037_create_tx_to_prefunded_account_has_no_new_account_charge)
{
    ...
    expect.gas_used = AMSTERDAM_CREATE_INTRINSIC;
    expect.state_gas = 0;
