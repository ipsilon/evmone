...
TEST_F(state_transition, access_list_floor_amsterdam)
{
    ...
    tx.access_list = {{To, {}}};
    // intrinsic = 15000 base (12000 + 3000 recipient) + 100*4 data + (2900 + 1280 access list)
    //           = 19580
...
{
    // EIP-2780/7976: the gas limit must cover the floor (22680, above the 19580 intrinsic); one
    // below is rejected.
    rev = EVMC_AMSTERDAM;
    tx.to = To;
    tx.data = bytes(100, 0x00);
