...
{
    // An access list may name a precompile together with storage keys (EIP-2930). access_account()
    // creates no state entry for a precompile, so warming the keys must materialize it explicitly.
    rev = EVMC_OSAKA;
    tx.to = To;
