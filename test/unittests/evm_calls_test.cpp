...
TEST_P(evm, create)
{
    ...
    // The VM computes the created address itself (from the sender and its nonce).
...
TEST_P(evm, create_failure)
{
    ...
    for (auto op : {OP_CREATE, OP_CREATE2})
    {
        // The VM computes the created address itself (empty initcode, salt 0).
...
{
    rev = EVMC_LONDON;
    // The first CREATE pushes its (VM-computed, nonzero) address, which the
    // second CREATE then pops as its value argument - the balance must cover it.
