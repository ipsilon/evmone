# EIP-7708 Update Proposal: Replace Burn Events with State-Preserving SELFDESTRUCT Finalization
...
- **Burn logs** — for ETH that leaves the world state. Defined for two cases:
  ...
  2. Any nonzero-balance account marked for deletion, at the time of removal during transaction finalization.

The **burn log paths are exotic**: post-EIP-6780, the only accounts that can be marked for deletion are contracts created in the same transaction, and the finalization case further requires that ETH be sent to such an account after it has already self-destructed. An examination of a sample of Ethereum mainnet blocks (30 consecutive `SELFDESTRUCT` opcode executions) showed **zero occurrences** of either condition: all observed `SELFDESTRUCT` calls had zero balance and distinct beneficiaries.
...
At the point of transaction finalization, for each account that was marked for deletion during the transaction, the following mutations are applied to the world state instead of removing the account:
...
When a contract created in the same transaction executes `SELFDESTRUCT` with itself as the beneficiary, the opcode marks the account for deletion as before. The balance is preserved — no inline zeroing is required — and will be visible after finalization via the rules above (the account remains alive with its balance if nonzero, or is removed if zero).

### ETH transfer log (unchanged from EIP-7708)

A `LOG3` from the system address is emitted for:
...
Topics and data encoding are as defined in EIP-7708.
...
- If the self-destructed contract received additional ETH after `SELFDESTRUCT` and before transaction end, that balance remains as a free-standing balance-only account (previously: the balance was burned).
...
- In all other cases, the net state diff is identical to EIP-6780: the account is removed (because balance is zero and the EIP-161 empty-account rule applies).

### Gas accounting

State gas accounting (EIP-8037) that currently refunds state gas when a just-created account is destroyed must be updated: accounts retained with nonzero balance should not receive that refund, while accounts that are removed due to the empty-account rule should receive it as before.
...
The following EEST test categories require updates:

- `burn_logs/finalization_burn_logs`: expected logs must no longer include burn entries; expected post-state must include the balance-only account (or reflect the EIP-161 empty-account removal if balance is zero).
- `burn_logs/selfdestruct_to_self_same_tx`: expected logs must no longer include burn entries; expected post-state must reflect the preserved balance.
- Existing `transfer_logs/*` tests are unaffected.
